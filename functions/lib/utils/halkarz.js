...
exports.discoverIposFromHalkarz = discoverIposFromHalkarz;
...
function extractFieldValue(html, labels) {
    const plain = (0, helpers_1.stripTags)(html);
    for (const label of labels) {
        ...
        const plainValue = (0, helpers_1.stripTags)((plainMatch?.[1] || plainMatch?.[2] || "").trim());
        if (plainValue)
            return plainValue;
        ...
        if (rowMatch?.[1]) {
            const value = (0, helpers_1.stripTags)(rowMatch[1]);
            if (value)
                return value;
        }
        ...
        if (inlineMatch?.[1]) {
            const value = (0, helpers_1.stripTags)(inlineMatch[1]);
            if (value)
                return value;
        }
    }
...
async function fetchIpoDetail(link) {
    const html = await (0, helpers_1.withRetry)(async () => {
        ...
        if (!res.ok || /Just a moment/i.test(text)) {
            ...
            if (!proxy.ok)
                throw new Error(`halkarz detail HTTP ${res.status}`);
            return proxy.text();
        }
        return text;
    }, 2);
...
function parseFromMarkdown(md, maxItems) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!linkMatch || !tickerMatch)
            continue;
        const link = (0, helpers_1.decodeHtmlEntities)(linkMatch[1]);
        const ticker = tickerMatch[1].toUpperCase();
        const slug = (0, helpers_1.slugFromUrl)(link);
        ...
        if (/talep\s*toplan/i.test(chunk))
            statusText = "Talep Toplanıyor";
        else if (/basvuru|başvuru/i.test(chunk))
            statusText = "Başvuru Açık";
        else
            statusText = "Duyuru";
        for (let j = i; j < Math.min(i + 8, lines.length); j++) {
            ...
            if (titleMatch) {
                companyName = (0, helpers_1.stripTags)(titleMatch[1]);
            }
            if (!dateText && /\d{4}/.test(l) && /(ocak|şubat|subat|mart|nisan|mayıs|mayis|haziran|temmuz|ağustos|agustos|eylül|eylul|ekim|kasım|kasim|aralık|aralik)/i.test(l)) {
                dateText = (0, helpers_1.stripTags)(l);
            }
        }
        if (!companyName) {
            const fallbackName = line.match(/\"([^\"]{4,120})\"\)\s*[A-Z0-9]{3,7}\s*$/);
            companyName = (0, helpers_1.stripTags)(fallbackName?.[1] || ticker);
        }
        seen.add(slug);
...
async function discoverIposFromHalkarz(maxItems = 40) {
    ...
    for (const block of blocks) {
        ...
        const link = linkMatch?.[1] ? (0, helpers_1.decodeHtmlEntities)(linkMatch[1]) : "";
        const companyNameRaw = linkMatch?.[2] ? (0, helpers_1.stripTags)(linkMatch[2]) : "";
        const companyName = companyNameRaw || "Bilinmeyen Sirket";
        const ticker = upper((0, helpers_1.stripTags)(tickerMatch?.[1] || "")).replace(/[^A-Z0-9]/g, "");
        const dateText = (0, helpers_1.stripTags)(dateMatch?.[1] || "");
        const statusText = (0, helpers_1.stripTags)(statusMatch?.[1] || "");
        const slug = (0, helpers_1.slugFromUrl)(link);
        if (!link || !slug || !ticker)
            continue;
        let detail = {};
